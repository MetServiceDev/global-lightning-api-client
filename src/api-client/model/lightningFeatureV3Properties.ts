...
export interface LightningFeatureV3Properties {
	/**
	 * an integer value that when left-padded with zeroes to six digits and appended to the dateTime gives the time of strike with nanosecond- resolution
	 */
	nanosecondsRemainder?: number;
	/**
	 * a decimal value of the chi-squared confidence value for the strike
	 */
	sensorChiSquared?: number;
	/**
	 * an integer value representing the number of degrees of freedom used when calculating the strike location
	 */
	sensorDegreesFreedom?: number;
	/**
	 ...
	 */
	sensorPeakTime?: number;
	/**
	 * a decimal value of the signal normalized for range
	 */
	sensorRangeNormalisedSignal?: number;
	/**
	 * an integer count of the number of sensors that reported the strike
	 */
	sensorReportingSensors?: number;
	/**
	 * Risetime of the waveform in microseconds
	 */
	sensorRiseTime?: number;
	/**
	 * The provider that reported this strike
	 */
	source: LightningFeatureV3Properties.SourceEnum;
	/**
	 * Geometric Dilution of Precision. A value that is calculated from the location of the lightning strike with respect to the location of the sensors used in its detection. A smaller GDOP means a more accurate location calculation. Only reported by certain providers.
	 */
	GDOP?: number;
	/**
	 * Date and time at which the lightning is reported to have happened. Given by all providers, but the *precise* meaning of the given time is at their discretion.
	 */
	dateTime: string;
	/**
	 * Date and time at which the lightning is reported to have happened, represented as UNIX Epoch time (seconds since epoch).
	 */
	unixTime: number;
	/**
	 * The error ellipse azimuth of the major axis in degrees. 0° represents north, +90° represents east, -90° represents west, etc. Since an ellipse is symmetrical the centre, an azimuth of +315° (or +135°) is the same ellipse as the one having an azimuth of ±45°. Only reported by certain providers.
	 */
	ellipseBearing?: number;
	/**
	 * Length of the major axis of an error ellipse, in kilometres. The error ellipse is centred at the location of the lightning strike. The error ellipse represents an area with a 95% confidence level that the lightning strike did occur within the area. Only reported by certain providers.
	 */
	ellipseMajorAxis?: number;
	/**
	 * Length of the minor axis of an error ellipse, in kilometres. The error ellipse is centred at the location of the lightning strike. The error ellipse represents an area with a 95% confidence level that the lightning strike did occur within the area. Only reported by certain providers.
	 */
	ellipseMinorAxis?: number;
	/**
	 * Stroke amplitude value (\"size\"), in kiloamperes. Not all providers have a method of accurately measuring this value, so it is nullable.
	 */
	kA?: number;
	/**
	 * The type of strike, whether cloud-to-cloud or cloud-to-ground.
	 */
	strikeType: LightningFeatureV3Properties.StrikeTypeEnum;
}
export namespace LightningFeatureV3Properties {
	export type SourceEnum = 'toa' | 'transpower' | 'mock';
	export const SourceEnum = {
		Toa: 'toa' as SourceEnum,
		Transpower: 'transpower' as SourceEnum,
		Mock: 'mock' as SourceEnum,
	};
	export type StrikeTypeEnum = 'CLOUD' | 'GROUND';
	export const StrikeTypeEnum = {
		CLOUD: 'CLOUD' as StrikeTypeEnum,
		GROUND: 'GROUND' as StrikeTypeEnum,
	};
}
